class ChargesController < ApplicationController
  ...
  def show
    ...
    if @charge.is_current?
      ...
      @entries_gaunt=@charge.entries.where('dist_gauntlet IS NOT NULL and is_bikes=false').order(position_gauntlet: :asc)

      @entries_tsetse1=@charge.entries.where('dist_tsetse1 IS NOT NULL and position_tsetse1 IS NOT NULL').order(position_tsetse1: :asc)
